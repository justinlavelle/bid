...
foreach($data as $key => $value){
	...
	if(empty($result)) {
		...
		// Get the bids amount
		//$amount = mysql_fetch_array(mysql_query("SELECT SUM(debit) as amount FROM bids where user_id=".$user_id." and auction_id=".$auction['id']." and type='bid'"), MYSQL_ASSOC);
		//echo "SELECT SUM(debit) as amount FROM bids where user_id=".$user_id." and auction_id=".$auction['id']." and type='bid'";
		...
		$bid = mysql_fetch_array(mysql_query("SELECT b.id, b.user_id, b.description, b.created, u.username, u.avatar FROM bids b, users u WHERE b.credit = 0 AND b.user_id = u.id AND b.auction_id = ".$value." ORDER BY b.id DESC LIMIT 1"), MYSQL_ASSOC);
		...
		if(!empty($bid['username'])) {
			...
			$result['LastBid']['username'] = 'Chưa có ai';
		}
		...
		if(!empty($_GET['histories'])) {
			...
			$bidHistories = mysql_query("SELECT b.id, b.user_id, b.description, b.debit, b.created, u.username, u.avatar FROM bids b, users u WHERE b.credit = 0 AND b.user_id = u.id AND b.auction_id = ".$auction['id']." ORDER BY b.id DESC LIMIT ".$config['App']['bidHistoryLimit']);
	...
	$result['Auction']['time_left']	= strtotime($result['Auction']['end_time']) - time()-2;
	...
	$timearray=split(':',getStringTime($result['Auction']['end_time']));
	if(!empty($config['App']['delayedStart'])) {
		...
		if ($result['Auction']['time_left']>10)
			$result['Auction']['end_time_string']   = '<div class="clock">'.$timearray[0].'</div> <div class="clock">'.$timearray[1].'</div><div class="clock last">'.$timearray[2].'</div>';
		else 
			$result['Auction']['end_time_string']   = '<div class="clock rush">'.$timearray[0].'</div> <div class="clock rush">'.$timearray[1].'</div><div class="clock rush last">'.$timearray[2].'</div>';
	}
	...
	if(!empty($user_id)){
		require_once '../database.php';
		$bidbutler = mysql_fetch_array(mysql_query("SELECT id, minimum_price, maximum_price, bids FROM bidbutlers WHERE auction_id = $value AND user_id = $user_id AND active=1"), MYSQL_ASSOC);
		if(!empty($bidbutler))
			$result['Bidbutler']=$bidbutler;
		else
			$result['Bidbutler']=null;
	} else{
		$result['Bidbutler']=null;
	}
