	...
	<tr<?php echo $class;?>>
		...
		<td class="actions">
			...
			<?php if(!empty($auction['Bid'])) : ?>
				...
				/ <?php echo $html->link("Trả lại XU cho người thắng cuộc", array('action' => 'refundwinner', $auction['Auction']['id']), null, sprintf(__('Are you sure you want to refund ALL the bids for this auction titled: %s?  This will delete ALL the bids on the auction to date.  If the auction has a winner, you should update the status to \'Refunded\' for the auction if you are refunding the winner.', true), $auction['Product']['title'])); ?>
				/ <?php echo $html->link("Trả lại 10% XU cho mọi người chơi", array('controller'=> 'bids', 'action' => 'errorrefund', $auction['Auction']['id']), null, 'Chắc chắn?');?>
