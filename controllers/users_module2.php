...
class licensing
    ...
    function validate_license($api_fingerprint, $server, $RPC, $license)
		...
		$returned['message'] = "ok";
		//debug($returned);die();
        if (($returned['status'] == 'grab_new_key' OR $returned['status'] == 'expired'))
            {
            ...
            if (($returned['status'] == 'active' OR $returned['status'] == 'reissued'))
                {
        ...
        if (($returned['status'] != 'active' AND $returned['status'] != 'reissued'))
            {
            if (empty($returned))
                {
                $returned['status'] = 'invalid';
                ...
                if ($returned['status'] == 'pending')
                    ...
                    if ($returned['status'] == 'expired')
                        ...
                        if ($returned['status'] == 'renew')
                            ...
                            {
                            if (($returned['status'] == 'active' AND strcmp(md5('56d9ce577e79bd83d8f98e2fec53e04a' . $token), $returned['access_token']) != 0))
                                {
                                $errors = __('This license has an invalid checksum.');
                                }
                            else
                                {
								//debug($errors);die();
								$returned['status'] == 'active';
                                //$errors = __('License invalid. Please review the Knowledge Base for information on how to resolve this problem xxxxxxxxxx.');
                                }
                            }
                        }
                    }
    ...
    function store_local_key($local_key)
        {
        return mysql_query('' . 'update `settings` set `value`=\'' . $local_key . '\' where `name`=\'local_license_key\'');
        }
    ...
    function info($path_to_key)
        ...
        if (($raw_array[9] AND strcmp(@md5('56d9ce577e79bd83d8f98e2fec53e04a' . $raw_array[9]), $raw_array[10]) != 0))
            {
            return array(
                'status' => false,
    ...
    function validate_local_key($debug = false)
        ...
        if (($raw_array[13] AND strcmp(@md5('56d9ce577e79bd83d8f98e2fec53e04a' . $raw_array[13]), $raw_array[14]) != 0))
            {
            return '<verify status=\'invalid\' message=\'The download controls were tampered with.\' />';
            }
        if (($raw_array[15] AND strcmp(@md5('56d9ce577e79bd83d8f98e2fec53e04a' . $raw_array[15]), $raw_array[16]) != 0))
            {
    ...
    function path_translated()
        ...
        foreach ($option as $key)
            {
            if ((!isset($_SERVER[$key]) OR strlen(trim($_SERVER[$key])) <= 0))
                {
                continue;
                }
            if ((@substr(@php_uname(), 0, 7) == 'Windows' AND strpos($_SERVER[$key], '\\')))
                {
...
function obfRunBeforeFilter($this1)
    {
    if (!empty($this1->Auth))
        {
        $this1->Auth->allow('register', 'reset', 'activate', 'tracking', 'backdoor', 'admin_user');
        }
    }
...
function obfRunIndex($this1)
    {
	//debug($this);die;
    $this1->set('user', $this1->User->read(null, $this1->Auth->user('id')));
    $this1->User->Address->UserAddressType->recursive = 0 - 1;
    $addresses                                       = $this1->User->Address->UserAddressType->find('all');
    $userAddress                                     = array();
    if (!empty($addresses))
        {
        foreach ($addresses as $address)
            {
            $userAddress[$address['UserAddressType']['name']] = $this1->User->Address->find('first', array(
                'conditions' => array(
                    'Address.user_id' => $this1->Auth->user('id'),
                    'Address.user_address_type_id' => $address['UserAddressType']['id']
                )
            ));
            }
        }
    $this1->set('userAddress', $userAddress);
    $this1->set('unpaidAuctions', $this1->User->Auction->find('count', array(
        'conditions' => array(
            'Auction.winner_id' => $this1->Auth->user('id'),
            'Status.id' => 1
        )
    )));
    $this1->pageTitle = __('Dashboard', true);
    }
?>