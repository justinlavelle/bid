		...
		<div class="content">
			...
			<div id="rightcol">
				...
				<fieldset>
					...
					<?php $bidbutlerTime = $this->requestAction('/settings/get/bid_butler_time'); ?>
					<p><?php __('Your bids will be used when the counter reaches');?> <?php echo $bidbutlerTime; ?> <?php __('seconds.');?></p>
			
					<?php if($appConfigurations['bidButlerType'] == 'advanced') : ?>
						<p><?php __('The minimum price must be at least');?> <?php echo $number->currency(1, $appConfigurations['currency']);?> <?php __('higher than the current bid.');?></p>
					<?php endif; ?>
				<?php
					if($appConfigurations['bidButlerType'] !== 'simple') :
