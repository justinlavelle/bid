...
class DaemonsController extends AppController {
	...
	function winner($auction_id) {
		...
		$auctions = $this->Auction->AuctionEmail->find('all', array(
			'contain' => array('Auction' => array('Product', 'Winner')),
			'conditions' => array('AuctionEmail.auction_id'),
		));
			...
			foreach ($auctions as $auction) {
				if(!empty($auction['Auction']['closed'])) {
					...
					if($this->appConfigurations['emailWinner'] == true) {
						...
						$data['subject'] 			   = "Bạn đã chiến thắng một phiên đấu giá "+$data['Product']['name'];
					...
					$data['subject'] 			   = "Bạn đã bán được sản phẩm "+$data['Product']['name'];
