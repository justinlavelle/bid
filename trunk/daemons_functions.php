...
function get($name = null, $auction_id = null, $cache = true) {
	...
	if($config['App']['bidIncrements'] == 'single') {
		...
		$settings = mysql_fetch_array(mysql_query("SELECT `bp_cost` as `bid_debit`, `price_step` as `price_increment`, `time_increment` FROM auctions where id = ".$auction_id), MYSQL_ASSOC);
...
function checkCanClose($id, $isPeakNow, $timeCheck = true) {
	...
	$auction = mysql_fetch_array(mysql_query("SELECT a.id, a.end_time, a.max_end_time, a.max_end, a.peak_only, a.price, a.minimum_price, a.reverse, a.autobids, a.leader_id, p.autobid, p.autobid_limit, a.random FROM auctions a, products p WHERE a.id = $id AND a.product_id = p.id"), MYSQL_ASSOC);
...
function bid($data = array(), $autobid = false, $bid_description = null) {
	...
	if(empty($data['user_id'])) {
		$message = __('Bạn chưa đăng nhập', true);
		$canBid = false;
		$data['user_id'] = 0;
	}
	...
	$auction = mysql_fetch_array(mysql_query("SELECT id, product_id, start_time, end_time, price, peak_only, closed, minimum_price, autobids, max_end, max_end_time, penny, reverse, beginner FROM auctions WHERE id = $auction_id"), MYSQL_ASSOC);
	...
	if(!empty($auction)){
		...
		if(!empty($config['App']['freeAuctions'])) {
			$product_id = $auction['product_id'];
			$product = mysql_fetch_array(mysql_query("SELECT free FROM products WHERE id = $product_id"), MYSQL_ASSOC);
		...
		if(!empty($config['App']['limits']['active'])) {
			...
			if($limits_exceeded == false) {
				...
				$message = __('Bạn đã bid quá số lượt cho phét trong phiên đấu giá này', true);
				$canBid = false;
			}
		}
		...
		if ($auction['beginner'] && !$autobid) {
			...
			if ($won_auctions_count>0) {
				...
				$message = __('Phiên đấu giá này chỉ dành cho thành viên mới', true);
				$canBid = false;
			}
		...
		if((!empty($auction['closed']) || strtotime($auction['end_time']) <= time()) && $autobid == false && !$data['bid_butler']) {
			$message = __('Phiên đấu giá đã kết thuc', true);
		...
		if ($auction['reverse']==1 && $auction['price']<=0) {
			$message = __('Phiên đấu giá đã kết thúc', true);
		...
		if(!empty($auction['start_time'])) {
			if(strtotime($auction['start_time']) > time()){
				$message = __('Phiên đấu giá chưa bắt ', true);
		...
		if(!empty($auction['peak_only'])){
			if(empty($data['isPeakNow'])){
				$message = __('Phiên đấu giá chỉ cho phét đặt vào phút cuối', true);
		...
		if(!empty($latest_bid) && $latest_bid['user_id'] == $data['user_id']) {
			$message = __('Bạn đang là người bid cuối cùng', true);
		...
		if($canBid == true) {
			...
			if($balance >= $data['bid_debit']) {
				...
				if (!$max_time_on or ($max_time_on && $invoke_max_time)) {
					if(!empty($config['App']['delayedStart'])) {
						...
						if(empty($latest_bid)) {
							...
							if (strtotime($auction['end_time'])- time() <10)
				...
				if(!empty($data['bid_butler']) && $config['App']['bidButlerType'] == 'advanced') {
					$bid['debit'] = 0;
				} else {
				...
				if(!empty($data['bid_butler'])) {
					...
					if(!empty($bidbutler)){
						if($bidbutler['bids'] >= $data['bid_debit']) {
							...
							// If bidbutler<bp_cost, inactive bidbutler
							mysql_query("UPDATE bidbutlers SET active=0 WHERE id=".$bidbutler['id']." AND bids<(SELECT bp_cost FROM auctions WHERE auctions.id=bidbutlers.auction_id)");
						} else {
				...
				if($success == 1) {
					...
					mysql_query("INSERT INTO bids (user_id, auction_id, description, credit, debit, created, modified, type) VALUES ('".$bid['user_id']."', '".$bid['auction_id']."', '".$bid['description']."', '".$bid['credit']."', '".$bid['debit']."', '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', '".$bid['type']."')");
					...
					$message = __('Bid đã được cập nhật', true);
				} else {
					logIt($auction['id'].' '.$data['user_id'].' - cantBid: unknown bid error');
					$message = __('Có lỗi trong việc đặt bid', true);
				}
				...
				$message = __('Bạn không đủ BP', true);
...
function limitsCanBid($auction_id = null, $user_id = null, $product_id = null, $autobidder = false) {
	...
	$sql = mysql_query("SELECT leader_id FROM auctions WHERE leader_id = $user_id AND end_time > '$expiry_date'");
	...
	$sql = mysql_query("SELECT leader_id FROM auctions a, products p, limits l WHERE a.product_id = $product_id AND a.leader_id = $user_id AND end_time > '$expiry_date' AND p.id = a.product_id AND l.id = p.limit_id");
	$productTotal   = mysql_num_rows($sql);

	$limit = mysql_fetch_array(mysql_query("SELECT l.limit, l.id FROM limits l, products p WHERE l.id = p.limit_id AND p.id = $product_id"), MYSQL_ASSOC);
...
function canBidBuddy($bidbutler) {
	global $config;
	if (strtotime($bidbutler['end_time'])- time() >10)
		return false;
	if ($bidbutler['reverse']) {
		...
		if ($bidbutler['price'] >= $bidbutler['minimum_price'] &&
			   ...
			   $config['App']['bidButlerType'] == 'simple' ||
			   $bidbutler['fixed'] == 1) {
			logIt($bidbutler['auction_id']. '; canBidBuddy: false');
			return true;
		}
	}
	return false;
...
function autorelist($auction = array()) {
	...
	$productSql = mysql_query("SELECT id FROM auctions WHERE closed = 0 AND active = 1 AND product_id = ".$auction['product_id']);
...
function logIt($msg) {
	global $log_file;
	if (!isset($log_file) or !$log_file) return false;
