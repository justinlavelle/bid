...
function getStringTime($timestamp){
	$diff 	= strtotime($timestamp) - time()-2;
...
/*	$day    = floor($diff / 86400);
	...
	return trim($hour.':'.$minute.':'.$second);
}
...
function currency($number, $currency = 'VIE', $options = array()) {
	/*
	global $config;
...
function niceShort($dateString = null, $userOffset = null) {
	...
	if (_isToday($date)) {
		$ret = "Hôm nay, " . date($timeFormat, $date);
	} elseif (_wasYesterday($date)) {
		$ret = "Hôm qua, " . date($timeFormat, $date);
	} else {
