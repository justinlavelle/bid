...
<?php echo $form->create(null, array('url' => '/admin/bids/add/'.$user['User']['id']));?>
	...
	?>
		<div class="input select">
			<label for="BidType"> <?php echo __('Type',true)?> * </label>
			<select id="BidType" name=data[Bid][type]>
				<option> <?php echo __('Bid Charge',true)?> </option>
				<option> <?php echo __('Bid Rewards',true)?> </option>
			</select>
		</div>
	<?php
