<?php
	class Product extends AppModel {
		...
		function afterSave(){
			if(!empty($this->data['Product']['meta_keywords'])){
				$tag_arr = explode(',',$this->data['Product']['meta_keywords']);
				foreach($tag_arr as $key=>$value)
				{
					$tag_arr[$key]=trim($value);
				}
				
				//get product_id for ProductTag
				if(!$this->id){
					//insert data
					$data['ProductTag']['product_id']=$this->getInsertID();
				}
				else{
					//update data
					$data['ProductTag']['product_id']=$this->id;
				}
				
				$this->ProductTag->deleteAll(array(
					'product_id' => $data['ProductTag']['product_id']
				));
				
				//for each tag
				foreach($tag_arr as $tag_name):
					$data['Tag']['tag_name']=$tag_name;
					
					$ftag=$this->ProductTag->Tag->find('first',array(
						'conditions' => array('tag_name' => $data['Tag']['tag_name']),
					));
					if(!$ftag){
						//tag_name isn't in Tags table
						
						//Create new Tag
						$this->ProductTag->Tag->create();
						$this->ProductTag->Tag->save($data);
						
						//get new inserted ID and insert to Product Tag
						$data['ProductTag']['tag_id']=$this->ProductTag->Tag->getInsertID();
						$this->ProductTag->create();
						$this->ProductTag->save($data);
					}
					else{
						//tag_name exist in Tags table
						$product_tag=$this->ProductTag->find('first',array(
							'conditions' => array('tag_id' => $ftag['Tag']['id'], 'product_id' => $data['ProductTag']['product_id']),
						));
						if(!$product_tag){
							$data['ProductTag']['tag_id']=$ftag['Tag']['id'];
							$this->ProductTag->create();
							$this->ProductTag->save($data);	
						}
					}
