...
<!-- Free guide start --><?php if(!empty($auction['Product']['free'])):?>                       
...
<a href="#" id="trigger_reverse"><?= $html->image('badge/reverse_icon.png', array('width'=>"50", 'height'=>"50", 'border'=>"0", 'alt'=>"Reverse bid auction", 'title'=>"Reverse bid auction"))?></a>
	<div class="tooltip">
	<p><label>Reverse auctions <strong class="orange2">cost nothing to enter.</strong> Ever! Reverse acutions are worry free bidding!</label></p>
				...
				<div class="content auction-item" title="<?php echo $auction['Auction']['id'];?>" id="auction_<?php echo $auction['Auction']['id'];?>" style="padding:0;">
					<div class="sub-col1"<?php if ($buy_it_now) { ?> style="height:365px"<?php } ?>>
						...
						<?php if(empty($auction['Auction']['isClosed']) && empty($auction['Auction']['isFuture']) && empty($auction['Product']['fixed'])):?>
							...
							<?php if(!empty($auction['Product']['free'])):?>Free listing! Costs nothing. <br />
							<?php endif; ?>
			...
			<p><?= __('Please enter a range') ?></p>
				<?php echo $form->create('Bidbutler', array('url' => '/bidbutlers/add/'.$auction['Auction']['id']));?>
					...
					<br style="clear:both">
						...
						?>
					<input name="data[Bidbutler][maximum_price]" type="hidden" value="-1" id="BidbutlerMaximumPrice" />
						<?php
					}
					?>
		...
		<div class="content" style="padding:10px 25px; margin-bottom:10px;">
			<div class="col1">
				...
				<dd><?php if(!empty($auction['Product']['free'])):?>
						...
						<?php endif; ?>
						<?php if(empty($auction['Product']['free']) && empty($auction['Product']['fixed']) && empty($auction['Auction']['reverse']) && empty($auction['Auction']['penny']) && empty($auction['Auction']['nail_bitter'])  && empty($auction['Auction']['beginner']) && empty($auction['Auction']['peak_only'])):?>
